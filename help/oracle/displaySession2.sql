...
break on "USERNAME" on osuser on machine on status
SELECT  SUBSTR(NVL(DECODE(sess.username, null, SUBSTR(proc.program,
			INSTR(proc.program, '(',1)), sess.username), '**Kill?**'), 1, 14) "USERNAME", 
        sess.osuser,
        sess.machine,
        sess.status,
		chr(10),
        NVL(TO_CHAR(sess.sid), '*Kill?*') "SID", 
		sess.serial#,
        proc.spid "UNIX PID",
	...
	TRUNC(last_call_et/(60*60*24)) || ':' || 
        ...
        --bandi_pkg.j_sec_to_ddhhmiss(last_call_et) "IDLE_FOR",
FROM    v$session sess, v$process proc, audit_actions comm
